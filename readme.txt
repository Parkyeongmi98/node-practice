...
3. 패키지 설치
    ...
    3) 패키지 설치 실습
        $ npm i ejs         [local, general dependency](지역 설치, 일반 의존성)
        $ npm i -D nodemon  [local, dev dependency](지역 설치, 개발 의존성(-D))
        $ npm i -g gulp     [global, general dependency](전역 설치, 일반 의존성)
        $ npx gulp --version : 설치되있으므로 버전이 나옴
    4) 패키지 삭제
        $ npm un ejs        [local install 삭제]
        $ npm un nodemon    [local install 삭제]
        $ npm un -g gulp    [global install 삭제]
        $ npx gulp --version : 삭제되었으므로 다시 설치하겠냐고 물어봄

4. Node(JavaScript) Project 생성
    1) 프로젝트 생성(mkdir)
    2) 프로젝트 이동(cd)
    3) 프로젝트 초기화: 프로젝트 메니페스트(manifest) 파일인 packge.json를 생성
